...
exports.run = async (client, msg, args) => {  
  let u = msg.mentions.users.first() || msg.author;
        ...
        if(u.bot === true) {
                msg.channel.send("Botların seviyesi bulunmamaktadır!")
                return
        }
  var g = "50"
  ...
  var lvl = db.fetch(`seviye_${u.id + msg.guild.id}`);  
        ...
        ctx.textAlign = "right";
        ctx.fillText(`${xp || 0} / 300 XP`, 670, 100);
  ctx.fillStyle = `#fcfdff`;
  ctx.font = 'bold 28px Impact';
        ctx.textAlign = "left";
        ctx.fillText(`${u.tag}`, 260, 150)
        ctx.beginPath();
        ctx.lineWidth = 9;
  ctx.fill()
        ctx.lineWidth = 8;
...
exports.help = {
  name: 'rank',
  description: 'Sunucu içindeki seviyenizi ve sıralamanızı gösterir.',
  usage: 'rank <@kişi> ya da rank'
};